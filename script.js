...
"411": {
  ...
  "Etre-01": {
    ...
    "Materials": ["Dragoon's Memory (Green)", "Spear Jadeite", "Prowling Wyvern Statue (Green)", "Monk's Memory (Green)", "Fire Alcryst (Green)", "Earth Alcryst (Green)"]
    },
  "Etre-02": {
    "Name":"Etre Training Quest II",
    "Materials": ["Fists Azurite", "Earth Alcryst (Blue)", "Monk's Memory (Blue)", "Praying Angel Statue (Blue)"]
    },
  "Etre-03": {
    "Name":"Etre Training Quest III",
    "Materials": ["Spear Azurite", "Fire Alcryst (Blue)", "Prowling Wyvern Statue (Blue)", "Dragoon's Memory (Blue)"]
    },
  "Etre-04": {
    "Name":"Etre Training Quest IV",
    "Materials": ["Fists Charoite", "Earth Alcryst (Purple)", "Praying Angel Statue (Purple)", "Monk's Memory (Purple)"]
    },
  "Etre-05": {
...
function refreshLocationInString(){
  ...
  let i = 0;
    for (let [key, value] of Object.entries(locationlist)) {
        ...
        // locationResultsInString[i][2] = 0;
        // locationResultsInString[i][3] = 0;
        // locationResultsInString[i][4] = 0;
        i += 1;
}
}

function refreshJobMemory() { // return an array of totalJobMemory with empty value except the first col
    let i = 0;
...
function refreshLocation() { // return an array of totalJobMemory with empty value except the first col
    let i = 0;
    for (let [key, value] of Object.entries(locationlist)) {
        locationResults[i] = [];
        locationResults[i][0] = key;
        locationResults[i][1] = 0;
        // locationResults[i][2] = 0;
        // locationResults[i][3] = 0;
        // locationResults[i][4] = 0;
        i += 1;
    }
}
...
const jobnames = ["Warrior", "Knight", "Soldier", "Ranger", "Monk", "Lancer", "Gunner", "Thief", "White Mage", "Black Mage", "Red Mage", "Green Mage", "Samurai", "Paladin", "Time Mage", "Spellblade", "Cleric", "Ninja", "Dragoon", "Winged One", "Gunbreaker", "Sorceress", "Lord", "White Mage of Lapis", "Holy Swordsman (unreleased)", "Viking", "Apprentice Warrior (unrelease)", "Fell Knight (unreleased)", "Double Gunner (unreleased)", "Assassin (unreleased)", "Grandshelt Knight (unreleased)", "Light Warrior (unreleased)", "Magician (unreleased)"]
...
//     jobOrbs[a][b] = "Squire";
//   }
//   if (jobname == "Fell Knight") {
//     statues[a][b] = "Wyvern";
//     weapons[a][b] = "Sword";
//     jobOrbs[a][b] = "Fell Knight";
//   }
...
function findLocation(drop) {
    ...
    for (let [key, value] of Object.entries(locationlist)) {
        // console.log(value.Materials.length)
        for (let i = 0; i < value.Materials.length; i++) {
            // console.log(value.Materials[i])
            // console.log("lala")
...
function presentByGrade() {
    ...
    for (x = 1; x < 5; x++) {
        ...
        if (totalMemory[x - 1] != 0) { // faint memory
            ...
            pic = `<img src="images/mat/${id}.png" id="${id}" onclick='blackOut("${id}")' class="material-icon" alt="${drop}" title="${drop}">`
            text += `       <tr ${styleHTML}>
                        <td>${pic}</td>
    ...
    for (i = 0; i < 20; i++) {
            ...
            for (x = 0; x < locationResults.length; x++) {
                if (locationResultsOld[x][0] == locationResults[i][0]) {
                    for (y = 2; y < locationResultsInString[x].length; y++) {
                        ...
                        pic = `<img src="images/mat/${link}.png" class="material-icon" alt="${locationResultsInString[x][y]}" title="${locationResultsInString[x][y]}">`
                        textbody += pic;
                    }
                    check = 1;
                }
      ...
      <thead class="thead-dark">
        <tr>
          ...
          <th scope="col">Amount matched</th>
